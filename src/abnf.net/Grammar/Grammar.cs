...
public sealed class Grammar(IEnumerable<GrammarRule> rules)
{
    ...
    public bool TryValidate(string input, string startRuleName, out int errorPosition, out string errorMessage)
    {
        ...
        {
            errorPosition = 1; // 1-based for user-facing messages
        ...
        {
            // Report the furthest position reached (convert from 0-based to 1-based)
            errorPosition = matchResult.FurthestPosition + 1;
            errorMessage = matchResult.FurthestErrorMessage;
            return false;
        }
        ...
        {
            errorPosition = matchResult.Position + 1; // 1-based for user-facing messages
